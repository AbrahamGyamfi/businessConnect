...
router.get('/unread-count', requireAuth, async (req, res) => {
  try {
    const userBusinesses = await db.select({ id: business.id }).from(business).where(eq(business.userId, req.userId!))
    if (!userBusinesses.length) { res.json({ count: 0 }); return }
...
router.post('/', async (req, res) => {
  try {
    ...
    // Look up business + owner for email notification (best-effort)
...
router.patch('/:id/read', requireAuth, async (req, res) => {
  try {
    ...
    const [biz] = await db.select().from(business).where(and(eq(business.id, inq.businessId), eq(business.userId, req.userId!)))
    if (!biz) { res.status(403).json({ message: 'Unauthorized' }); return }
